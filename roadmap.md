...
| Heterojen dizi literalinin eleman tipleri Sema'da hiç birleştirilmiyor (önceden var, 2026-07 final incelemesinde bulundu, izlemede) | `let a: [i32] = [1, "a"]` Sema'dan geçiyor; generik alanda (`Box { v: [1, "a"] }`) çıkarım ilk elemandan T=i32 alıyor, ikinci elemanı okumak SEGFAULT. Aynı sınıf: `P { x: 1, v: ["a","b"] }` — skaler alan T'yi i32'ye pinliyor, string elemanlar `[i32]`'ye yazılıyor. Kalıcı çözüm IRGen'de değil, Sema'da eleman-tipi birleştirme + uyumsuzluk diagnostiği | `TypeChecker::visitArrayLiteralExpr` (şu an elemanları ziyaret edip resolvedType bile atamıyor) |
