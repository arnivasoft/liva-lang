...
| Fonksiyon argümanları ve atamalar hiç tip denetimi görmüyordu (çözüldü 2026-07) | Sema: `checkAssignable` (eski `checkArrayElement`) argüman ve atama yollarına bağlandı — denetim yalnız bildirimi çözülebilen doğrudan çağrılarda çalışır, builtin/closure/dinamik dispatch sessiz kalır. IRGen: dönüşüm site sayımı yerine BOĞAZ NOKTASINDA, callee'nin FunctionType'ına göre (`coerceCallArgs`) — böylece `takeI64(5)`/`takeF64(3)`/`takeU8(200)` gibi bugüne dek LLVM verifier hatası veren çağrılar çalışıyor. Atama yolunda da skaler dönüşüm eklendi. Kapsam eki: hesaplanmış özellik setter'ı/willSet/didSet gözlemcileri/subscript setter'ı — `visitAssignExpr` içindeki `CreateCall` siteleri (argüman-döngüsü dışında, Görev 4'ün taramadığı) — aynı `coerceCallArgs` deseniyle düzeltildi; düzeltilmeden `c.v = 5` (hesaplanmış `i64` alan) `LLVM module verification failed: call void @Counter_set_v(ptr %c.ptr, i32 5)` veriyordu | `TypeCheckerCall.cpp` `checkCallArgTypes`, `TypeChecker.cpp` `visitAssignExpr`, `IRGen.cpp` `coerceCallArgs`, `IRGenCall.cpp` `visitAssignExpr` (setter/observer/subscript-setter `CreateCall` siteleri); 13 Sema + 6 koşum testi. Kapsam dışı kalanlar: native builtin argümanları, variadic paketleme, düz üye alan ataması (computed olmayan GEP store) |
