...
| Generik struct'ın `[T]` alanına literal init çöküyor (önceden var, 2026-07 final incelemesinde bulundu, izlemede) | `struct Box<T> { var v: [T] }` + `Box { v: [10,20,30] }` → SEGFAULT (0xC0000005); iç içe `[[T]]` varyantı "index out of bounds". Kök neden: `inferStructTypeArgs` yalnız tipi DOĞRUDAN `Named T` olan alanlardan çıkarım yapıyor, `[T]` (Array kind) alan atlanıyor → typeArgs boş → alan substitüsyonsuz monomorfize ediliyor (`Named T` opak ptr'a düşüyor, elemSize 8). Açık `Box<[i32]> { ... }` ise parse hatası | `IRGenMono.cpp:367-373` `inferStructTypeArgs`; generic-mono-pitfalls ailesi |
