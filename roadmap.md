...
| Aynı süreç içinde birden çok `CompilerInstance::compile()` çağrısı state kirliliği yaratıyor — ExamplesTest'in 9 örneği tek süreçte derlendiğinde `CliDemo` ~%10 oranda yanlış çıktı üretiyor (`getOption` yanlış değer); ctest büyük ölçüde etkilenmiyor (`gtest_discover_tests` test-başına taze süreç; 2026-07-24'te tam ctest koşularında Cli testleri (CliOptionEqualsSplitsOnFirstOnly, CliDashDash...) BİRKAÇ kez tek tek düştü — izole/tekrarlı koşular hep stabil, ardışık tam koşular çoğunlukla temiz; muhtemelen ayrı mekanizma (temp-dosya/AV/yük çekişmesi), ayrıca izole edilip incelenmeli); `SelfHostTest.StrSplit*` ve önceki Cli aynı-süreç flake'leriyle muhtemelen aynı sınıf (izlemede) | Kök neden izole edilmedi — compiler global/static state şüphesi | Keşif: ExamplesTest Task 4, reviewer 4/40 tekrar oranıyla doğruladı |
