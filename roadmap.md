...
| Top-level `var x = 0` (global değişken) livac'ı derleme sırasında SEGFAULT ettiriyor, diagnostik yok (önceden var, 2026-07 final inceleme probe'unda bulundu) | Global VarDecl codegen yolu fonksiyon bağlamı (builder insert block) olmadan çalışıyor olmalı | Minimal repro: dosya başına `var counter = 0` + boş main |
...
| Aynı süreç içinde birden çok `CompilerInstance::compile()` çağrısı state kirliliği yaratıyor — ExamplesTest'in 9 örneği tek süreçte derlendiğinde `CliDemo` ~%10 oranda yanlış çıktı üretiyor (`getOption` yanlış değer); ctest büyük ölçüde etkilenmiyor (`gtest_discover_tests` test-başına taze süreç; 2026-07-24'te tam ctest'te bir kez 2 Cli testinde tek-seferlik flake gözlendi — izole rerun 6/6 stabil, muhtemelen ayrı mekanizma: temp-dosya/AV çekişmesi); `SelfHostTest.StrSplit*` ve önceki Cli aynı-süreç flake'leriyle muhtemelen aynı sınıf (izlemede) | Kök neden izole edilmedi — compiler global/static state şüphesi | Keşif: ExamplesTest Task 4, reviewer 4/40 tekrar oranıyla doğruladı |
