...
| Fonksiyon ARGÜMANLARI ve ATAMALAR Sema'da hiç tip denetimi görmüyor (önceden var, 2026-07 final incelemesinde ölçüldü, izlemede) | `TypeCheckerCall.cpp:75` yalnız argüman SAYISINI denetliyor; `visitAssignExpr` yalnız mutability denetliyor. Skaler uyumsuzlukta LLVM verifier arka-durdurucu var ama iki DynArray aynı LLVM tipine sahip olduğundan dizilerde yok: `func take(xs: [i32])` + `take(mkStr())` temiz derleniyor; `var a: [i32] = [1,2,3]; a = mkStr()` derleniyor ve `a[0]` çöp basıyor (pointer'ın i32 okunması). `typesCompatible` derinliği yalnız VarDecl ve return yollarını kapattı | `TypeCheckerCall.cpp` argüman denetimi, `TypeChecker.cpp` `visitAssignExpr` |
