...
| `typesCompatible` diziler için SIĞ — hiçbir zaman elemana inmiyor — bu yüzden `let m: [[string]] = [[1, 2]]` temiz derleniyor; yeni anotasyon-yönlendirmeli denetim de bu körlüğü bir seviye devralıyor (izlemede) | `checkArrayElement` yalnız NÜMERİK hedef/eleman çiftlerinde özel mantık çalıştırıyor; ikisi de dizi olduğunda `typesCompatible(target, elemType)`'a düşüyor, o da yalnız üst seviyeyi (Array kind) karşılaştırıp iç elemanı hiç karşılaştırmıyor | `TypeChecker.cpp` `typesCompatible` (Array dalı) — özyinelemeli eleman karşılaştırması eksik |
