...
| Dolu `[[T]]` struct-literal ALAN init'i sessizce yanlış veri üretiyor — `Grid { rows: [[1,2],[3,4]] }` sonrası `g.rows[1]` bozuk, `g.rows[0]` doğru (maskeliyor) (önceden var, bu branch'ten bağımsız keşfedildi, izlemede) | Struct-literal alan init'i, `[[T]]` için izlenen 6 kayıt sitesinin (env `dynArrayElemLLVMType`/`deriveNestedDynArrayInner` kullanan yollar) HİÇBİRİNE dahil değil — ayrı bir kod yolu; elem-size/offset desync imzası (`last.length` beklenen 3 yerine 0 döndü) | Probe-doğrulandı (`scratchpad/probe9.liva`, görev-2 review turu) — kök neden dosya/satır olarak izole edilmedi, kod yolu envanteri gerekiyor |
...
| Variadic fonksiyon ÇALIŞTIRMASI heap corruption ile çöküyor (önceden var, 2026-07 final inceleme probe'unda bulundu, ba0be16'ya karşı bisect-doğrulamalı) | `func f(v: i32...)` + çağrı derleniyor ama koşumda 0xC0000374 — variadic parametre varDynArrayTypes'a borrow işareti OLMADAN kaydediliyor, scope temizliği çağıran-sahipli buffer'ı free ediyor olmalı; mevcut VariadicFunction testi yalnız parse+sema | `IRGenDecl.cpp:615-617` — for-in borrow fix'iyle aynı sınıf; ucuz aday |
